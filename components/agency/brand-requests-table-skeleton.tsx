  ...
  return (
    <div className="rounded-xl border border-border bg-background-surface shadow-sm">
      ...
      <div className="flex gap-4 border-b border-border px-5 py-2.5">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="h-4 w-16 animate-pulse rounded bg-background-muted" />
        ))}
      </div>
        ...
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="flex items-center gap-6 border-b border-border/30 py-4 last:border-0">
            ...
            <div className="flex gap-2">
              <div className="h-7 w-20 animate-pulse rounded-md bg-background-muted" />
              <div className="h-7 w-16 animate-pulse rounded-md bg-background-muted" />
            </div>
          </div>
        ))}
      </div>
