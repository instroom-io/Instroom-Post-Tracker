  ...
  const [{ data: campaigns }, { data: recentPosts }] =
    await Promise.all([
      ...
      supabase
        .from('posts')
        .select(
          'id, thumbnail_url, media_url, drive_file_id, platform, posted_at, influencer:influencers(tiktok_handle, ig_handle, youtube_handle)'
        )
        .eq('workspace_id', workspaceId)
    ...
    <>
      ...
      <div className="rounded-xl border border-border bg-background-surface shadow-sm">
        <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
          ...
          {(campaigns ?? []).length > 0 && (
            <Link
              href={`/${workspaceSlug}/campaigns/${(campaigns ?? [])[0].id}?tab=posts`}
